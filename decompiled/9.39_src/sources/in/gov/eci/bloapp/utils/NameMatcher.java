...
public class NameMatcher {
    ...
    public static String normalize(String name) {
        ...
        return name.toLowerCase().trim().replace("0", "o").replace("1", "l").replace("5", "s").replace("rn", "m").replaceAll("[^a-z ]", "").replaceAll("\\s+", " ").trim();
    }

    public static List<String> getTokens(String name) {
        String strNormalize = normalize(name);
        return strNormalize.isEmpty() ? Collections.emptyList() : Arrays.asList(strNormalize.split(" "));
    }

...
import in.gov.eci.bloapp.views.activity.newsir.activity.FormVerification.SIRSelectPhotoActivity;
import in.gov.eci.bloapp.views.activity.newsir.model.VerifyPayload;
import java.util.ArrayList;
import java.util.List;

/* JADX INFO: loaded from: /tmp/dex_9.68/classes4.dex */
public class SelectPhotoListAdapter extends RecyclerView.Adapter<holder> {
    List<VerifyPayload> al;
    private Context context;
    List<VerifyPayload> filteredAl;

    public SelectPhotoListAdapter(List<VerifyPayload> al, Context context) {
        this.al = al;
    ...
    public void onBindViewHolder(holder holder2, final int i) {
        ...
        holder2.viewDetails.setOnClickListener(new View.OnClickListener() { // from class: in.gov.eci.bloapp.views.activity.newsir.adapter.SelectPhotoListAdapter.1
            @Override // android.view.View.OnClickListener
            public void onClick(View view) {
                Intent intent = new Intent(SelectPhotoListAdapter.this.context, (Class<?>) SIRSelectPhotoActivity.class);
                intent.putExtra("data", SelectPhotoListAdapter.this.al.get(i));
                intent.putExtra("from", "photo");
                SelectPhotoListAdapter.this.context.startActivity(intent);
            }
        });

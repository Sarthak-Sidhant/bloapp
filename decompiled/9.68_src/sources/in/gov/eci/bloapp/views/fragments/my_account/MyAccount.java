...
public class MyAccount extends Hilt_MyAccount {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        String[] strArrSplit = this.totalPartNumber.replace(RegexMatcher.JSON_STRING_REGEX, StringUtils.SPACE).replace("[", "").replace("]", "").replace(StringUtils.SPACE, "").split(",");
    ...
    public void getBlo() {
        ...
        if (!TextUtils.isEmpty(SharedPref.getInstance(requireContext()).getUserRole())) {
            str = SharedPref.getInstance(requireContext()).getUserRole().equalsIgnoreCase("blos") ? "blos" : "blo";
        }
        ((UserClient) ApiClient.getClient(getContext()).create(UserClient.class)).getMyProfile(this.token, SharedPref.getInstance(requireContext()).getAtknBnd(), SharedPref.getInstance(requireContext()).getRtknBnd(), "BLOAPP", str, "ANDROIDMOB").enqueue(new AnonymousClass1());
    }
    ...
    class AnonymousClass1 implements Callback<JsonObject> {
        ...
        public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
            ...
            if (response.code() == 200) {
                ...
                MyAccount.this.fName = jsonObject.get("userFname").toString().replaceAll("^\"|\"$", "").replaceAll(" null | null", StringUtils.SPACE);
                MyAccount.this.lName = jsonObject.get("userLname").toString().replaceAll("^\"|\"$", "").replaceAll(" null | null", StringUtils.SPACE);
                ...
                if (MyAccount.this.bloDesignation == null || MyAccount.this.bloDesignation.equals("") || MyAccount.this.bloDesignation.equalsIgnoreCase("NULL")) {
                    MyAccount.this.name = MyAccount.this.fName + StringUtils.SPACE + MyAccount.this.lName;
                } else {
                    MyAccount.this.name = MyAccount.this.fName + StringUtils.SPACE + MyAccount.this.lName + "(" + MyAccount.this.bloDesignation + ")";
                }

                ...
                if (response.isSuccessful() && ((JsonArray) response.body()).size() > 0) {
                    ...
                    for (int i = 0; i < jsonArray.size(); i++) {
                        ...
                        if (H2HDETAILS.this.asmblyNO.equals(H2HDETAILS.this.acNumberOther) && H2HDETAILS.this.partNo.equals(H2HDETAILS.this.partNoOther)) {
                            ...
                            H2HDETAILS.this.stateNameForForm6B = H2HDETAILS.this.payloadContent.get("stateName").toString().replace(RegexMatcher.JSON_STRING_REGEX, "").trim().replace("[ ]+", StringUtils.SPACE);
                            H2HDETAILS.this.districtNameForForm6B = H2HDETAILS.this.payloadContent.get("districtValue").toString().replace(RegexMatcher.JSON_STRING_REGEX, "").trim().replace("[ ]+", StringUtils.SPACE);
                            H2HDETAILS.this.acNameForForm6B = H2HDETAILS.this.payloadContent.get("asmblyName").toString().replace(RegexMatcher.JSON_STRING_REGEX, "").trim().replace("[ ]+", StringUtils.SPACE);
                            H2HDETAILS.this.assemblyNoForForm6B = H2HDETAILS.this.payloadContent.get(H2HDETAILS.this.acNumberText).toString().replace(RegexMatcher.JSON_STRING_REGEX, "").trim().replace("[ ]+", StringUtils.SPACE);
                            H2HDETAILS.this.firstNameForForm6B = H2HDETAILS.this.payloadContent.get("applicantFirstName").toString().replace(RegexMatcher.JSON_STRING_REGEX, "").trim().replace("[ ]+", StringUtils.SPACE);
                            H2HDETAILS.this.lastNameForForm6B = H2HDETAILS.this.payloadContent.get("applicantLastName").toString().replace(RegexMatcher.JSON_STRING_REGEX, "").trim().replace("[ ]+", StringUtils.SPACE);
                            H2HDETAILS.this.epicIdForForm6B = H2HDETAILS.this.payloadContent.get("epicNumber").toString().replace(RegexMatcher.JSON_STRING_REGEX, "").trim().replace("[ ]+", StringUtils.SPACE);
                            H2HDETAILS.this.mobileNoForForm6B = H2HDETAILS.this.payloadContent.get("mobileNumber").toString().replace(RegexMatcher.JSON_STRING_REGEX, "").trim().replace("[ ]+", StringUtils.SPACE);
                            H2HDETAILS.this.emailIdForForm6B = H2HDETAILS.this.payloadContent.get(Constants.EMAIL_ID).toString().replace(RegexMatcher.JSON_STRING_REGEX, "").trim().replace("[ ]+", StringUtils.SPACE);
                            H2HDETAILS.this.stateCdForForm6B = H2HDETAILS.this.payloadContent.get("stateCd").toString().replace(RegexMatcher.JSON_STRING_REGEX, "").trim().replace("[ ]+", StringUtils.SPACE);
    ...
    public void renderingData1(JSONArray allHouses) {
        ...
        try {
            ...
            for (int i = 0; i < allHouses.size(); i++) {
                ...
                String strReplace = asJsonObject.get(this.houseNoString).toString().replace("^\"|\"$", "").replace("null", StringUtils.SPACE);
                String strReplace2 = asJsonObject.get(this.applicantNameString).toString().replace("^\"|\"$", "").replace("null", StringUtils.SPACE);
        ...
        class AnonymousClass2 implements Callback<JsonArray> {
            ...
            public void onResponse(Call<JsonArray> call, Response<JsonArray> response) {
                if (response.body() != null && ((JsonArray) response.body()).size() > 0) {
                    ...
                    if (arrayList.isEmpty()) {
                        H2HDETAILS.this.showDialog("No Record found with name: " + H2HDETAILS.this.firstname + StringUtils.SPACE + H2HDETAILS.this.lastname);

...
public abstract class Scheduler {
    ...
    public static abstract class Worker implements Disposable {
        ...
        public Disposable schedulePeriodically(Runnable runnable, long j, long j2, TimeUnit timeUnit) {
            ...
            EmptyDisposable emptyDisposableSchedule = schedule(new PeriodicTask(jNow + timeUnit.toNanos(j), runnableOnSchedule, jNow, sequentialDisposable2, nanos), j, timeUnit);
            if (emptyDisposableSchedule == EmptyDisposable.INSTANCE) {
                return emptyDisposableSchedule;
            }
            sequentialDisposable.replace(emptyDisposableSchedule);
    ...
    static final class DisposeTask implements Disposable, Runnable, SchedulerRunnableIntrospection {
        ...
        public void dispose() {
            if (this.runner == Thread.currentThread()) {
                NewThreadWorker newThreadWorker = this.w;
                if (newThreadWorker instanceof NewThreadWorker) {
                    newThreadWorker.shutdown();
                    return;
                }
            }
            this.w.dispose();

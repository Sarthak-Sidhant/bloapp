...
public class PSEClusterDetailsListAdapter extends RecyclerView.Adapter<holder> {
    ...
    public void onBindViewHolder(holder holder2, int i) {
        holder2.electorNametv.setText((TextUtils.isEmpty(this.al.get(i).getFirstName()) ? "" : this.al.get(i).getFirstName()) + " " + (TextUtils.isEmpty(this.al.get(i).getLastName()) ? "" : this.al.get(i).getLastName()));
        holder2.imageView9.setImageResource(R.drawable.blo_dummy_image);
        String presignedUrl = this.al.get(i).getPresignedUrl();
        if (!TextUtils.isEmpty(presignedUrl)) {
            Glide.with(this.context).load(presignedUrl).placeholder(R.drawable.blo_dummy_image).into(holder2.imageView9);
        } else {
            holder2.imageView9.setImageResource(R.drawable.blo_dummy_image);
        }

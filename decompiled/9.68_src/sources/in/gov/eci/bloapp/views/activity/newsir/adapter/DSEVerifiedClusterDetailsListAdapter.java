...
public class DSEVerifiedClusterDetailsListAdapter extends RecyclerView.Adapter<holder> {
    private String acNo;
    List<DSEVerifiedClusterDetailsPayload> al;
    private String atkband;
    CommomUtility commonUtilClass = new CommomUtility();
    private Context context;
    private String encodedImage;
    List<DSEVerifiedClusterDetailsPayload> filteredAl;
    ItemClickCallback itemClickCallback;
    private String partNo;
    private String rtkband;
    private String token;

    public DSEVerifiedClusterDetailsListAdapter(List<DSEVerifiedClusterDetailsPayload> al, Context context, ItemClickCallback itemClickCallback) {
        this.al = al;
    ...
    public holder onCreateViewHolder(ViewGroup viewGroup, int i) {
        return new holder(LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.dse_cluster_items, viewGroup, false));
    }
    ...
    public void onBindViewHolder(holder holder2, int i) {
        ...
        if (!TextUtils.isEmpty(this.al.get(i).getStateName())) {
            holder2.State_name.setVisibility(0);
            holder2.State_name.setText(this.context.getResources().getString(R.string.stateSingleRow) + StringUtils.SPACE + String.valueOf(this.al.get(i).getStateName()));
        } else {
            holder2.State_name.setVisibility(8);
        }
        ...
        if (!TextUtils.isEmpty(this.al.get(i).getBloActionTaken()) && this.al.get(i).getBloActionTaken().equalsIgnoreCase("form7")) {
            holder2.lv_watermark.setVisibility(0);
        } else if (!TextUtils.isEmpty(this.al.get(i).getBloActionTaken()) && this.al.get(i).getBloActionTaken().equalsIgnoreCase("noAction")) {
            holder2.lv_watermark.setBackground(this.context.getDrawable(R.drawable.verified));
            holder2.lv_watermark.setVisibility(0);
        } else {
    ...
    public void fun(ArrayList<DSEVerifiedClusterDetailsPayload> filteredAl) {
        this.al = filteredAl;

        ...
        if (i == 3 || i == 4 || i == 5) {
            if ((this.$mode == Mode.SINGLE || this.$mode == Mode.SINGLE_OR_DEFAULT) && this.seenValue) {
                ...
                if (this.$cont.isActive()) {
                    ...
                    cancellableContinuation3.resumeWith(Result.m5265constructorimpl(ResultKt.createFailure(new IllegalArgumentException("More than one onNext value for " + this.$mode))));
                    return;
                }
                return;
            }
    ...
    public void onComplete() {
        if (tryEnterTerminalState("onComplete")) {
            ...
            if (this.$mode == Mode.FIRST_OR_DEFAULT || this.$mode == Mode.SINGLE_OR_DEFAULT) {
                CancellableContinuation<T> cancellableContinuation2 = this.$cont;
                Result.Companion companion2 = Result.INSTANCE;
                cancellableContinuation2.resumeWith(Result.m5265constructorimpl(this.$default));
            } else if (this.$cont.isActive()) {
                CancellableContinuation<T> cancellableContinuation3 = this.$cont;
                Result.Companion companion3 = Result.INSTANCE;
                cancellableContinuation3.resumeWith(Result.m5265constructorimpl(ResultKt.createFailure(new NoSuchElementException("No value received via onNext for " + this.$mode))));
            }
        }
    }

...
public final class BroadcastChannelImpl<E> extends BufferedChannel<E> implements BroadcastChannel<E> {
        ...
        try {
            ...
            if (!isClosedForSend() || this.lastConflatedElement != BroadcastChannelKt.NO_ELEMENT) {
                if (this.lastConflatedElement != BroadcastChannelKt.NO_ELEMENT) {
                    subscriberConflated.mo6758trySendJP2dKIU(getValue());
                }
    ...
    public Object mo6758trySendJP2dKIU(E element) {
        ...
        try {
            ...
            if (!(list instanceof Collection) || !list.isEmpty()) {
                ...
                while (it.hasNext()) {
                    if (((BufferedChannel) it.next()).shouldSendSuspend$kotlinx_coroutines_core()) {
                        Object objM6783failurePtdJZtk = ChannelResult.INSTANCE.m6783failurePtdJZtk();
                        reentrantLock.unlock();
                        return objM6783failurePtdJZtk;
                    }
                }
            }
            if (this.capacity == -1) {
            ...
            while (it2.hasNext()) {
                ((BufferedChannel) it2.next()).mo6758trySendJP2dKIU(element);
            }
            Object objM6784successJP2dKIU = ChannelResult.INSTANCE.m6784successJP2dKIU(Unit.INSTANCE);
            reentrantLock.unlock();
            return objM6784successJP2dKIU;
        } catch (Throwable th) {
            reentrantLock.unlock();
            throw th;
        }

...
public class MigrationRepository {
    ...
    public void updatepersonalpage(String referenceNumber, String personal, int stepseq) {
        SQLiteDatabase writableDatabase = this.dbHandler.getWritableDatabase(BuildConfig.DEFAULT_KEY);
        try {
            Logger.e("TAG", "UPDATE DRAFT_FORMS SET PERSONAL_DETAILS =?, STEP_SEQUENCE =? WHERE FORM_REFERENCE_NUMBER =?");
            SQLiteStatement sQLiteStatementCompileStatement = writableDatabase.compileStatement("UPDATE DRAFT_FORMS SET PERSONAL_DETAILS =?, STEP_SEQUENCE =? WHERE FORM_REFERENCE_NUMBER =?");
            sQLiteStatementCompileStatement.clearBindings();
            sQLiteStatementCompileStatement.bindString(1, personal);
            sQLiteStatementCompileStatement.bindString(2, String.valueOf(stepseq));
            sQLiteStatementCompileStatement.bindString(3, referenceNumber);

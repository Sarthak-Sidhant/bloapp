package in.gov.eci.bloapp.views.activity.newsir.model;

/* JADX INFO: loaded from: /tmp/dex_9.39/classes4.dex */
public class Content {
    public String aadharStatus;
    public int acId;
    public int acNumber;
    public int age;
    public String applicantFirstName;
    public String applicantFirstNameL1;
    public String applicantFirstNameL2;
    public String applicantLastName;
    public String applicantLastNameL1;
    public String applicantLastNameL2;
    public String asmblyName;
    public String asmblyNameL1;
    public String country;
    public String createdDttm;
    public String disabilityAny;
    public String disabilityType;
    public String districtCd;
    public int districtId;
    public int districtNo;
    public String districtValue;
    public String districtValueL1;
    public String dob;
    public String dvoterStatusType;
    public String electorType;
    public String emailId;
    public String epicDatetime;
    public Long epicId;
    public String epicNumber;
    public String formReferenceNo;
    public String formType;
    public String fullName;
    public String fullNameL1;
    public String gender;
    public String genderL1;
    public String houseNumber;
    public String houseNumberL1;
    public String id;
    public boolean isActive;
    public boolean isDeleted;
    public String isForm8Migration;
    public String isLocomotorDisabled;
    public String isOnlyMobileUpdation;
    public String isSpeechHearingDisabled;
    public String isValidated;
    public String isVip;
    public String isVisuallyImpaired;
    public String isWheelchairRequired;
    public String lastUpdatedOn;
    public Object localityStreet;
    public Object localityStreetL1;
    public String mobileNumber;
    public String modifiedDttm;
    public String oldEpicNumber;
    public String otherDisability;
    public int partId;
    public String partLatLong;
    public String partName;
    public String partNameL1;
    public int partNumber;
    public int partSerialNumber;
    public String photo;
    public String pinCode;
    public String pollingDate;
    public String prlmntName;
    public Object prlmntNameL1;
    public String prlmntNo;
    public String processType;
    public String psbuildingName;
    public String pwd;
    public String pwdMarkingFormType;
    public String pwdMarkingRefNo;
    public String relationLName;
    public String relationLNameL1;
    public String relationName;
    public String relationNameL1;
    public String relationNameL2;
    public String relationType;
    public String relationTypeL1;
    public String relativeFullName;
    public String relativeFullNameL1;
    public int revisionId;
    public int revisionNo;
    public int sectionNo;
    public String stateCd;
    public int stateId;
    public String stateName;
    public String stateNameL1;
    public String statusType;
    public String tehsil;
    public String tehsilL1;
    public Object townVillage;
    public Object townVillageL1;

    public String getId() {
        return this.id;
    }

    public void setId(String id) {
        this.id = id;
    }

    public Long getEpicId() {
        return this.epicId;
    }

    public void setEpicId(Long epicId) {
        this.epicId = epicId;
    }

    public String getEpicNumber() {
        return this.epicNumber;
    }

    public void setEpicNumber(String epicNumber) {
        this.epicNumber = epicNumber;
    }

    public String getFormReferenceNo() {
        return this.formReferenceNo;
    }

    public void setFormReferenceNo(String formReferenceNo) {
        this.formReferenceNo = formReferenceNo;
    }

    public String getApplicantFirstName() {
        return this.applicantFirstName;
    }

    public void setApplicantFirstName(String applicantFirstName) {
        this.applicantFirstName = applicantFirstName;
    }

    public String getApplicantFirstNameL1() {
        return this.applicantFirstNameL1;
    }

    public void setApplicantFirstNameL1(String applicantFirstNameL1) {
        this.applicantFirstNameL1 = applicantFirstNameL1;
    }

    public String getApplicantFirstNameL2() {
        return this.applicantFirstNameL2;
    }

    public void setApplicantFirstNameL2(String applicantFirstNameL2) {
        this.applicantFirstNameL2 = applicantFirstNameL2;
    }

    public String getApplicantLastName() {
        return this.applicantLastName;
    }

    public void setApplicantLastName(String applicantLastName) {
        this.applicantLastName = applicantLastName;
    }

    public String getApplicantLastNameL1() {
        return this.applicantLastNameL1;
    }

    public void setApplicantLastNameL1(String applicantLastNameL1) {
        this.applicantLastNameL1 = applicantLastNameL1;
    }

    public String getApplicantLastNameL2() {
        return this.applicantLastNameL2;
    }

    public void setApplicantLastNameL2(String applicantLastNameL2) {
        this.applicantLastNameL2 = applicantLastNameL2;
    }

    public String getRelationName() {
        return this.relationName;
    }

    public void setRelationName(String relationName) {
        this.relationName = relationName;
    }

    public String getRelationNameL1() {
        return this.relationNameL1;
    }

    public void setRelationNameL1(String relationNameL1) {
        this.relationNameL1 = relationNameL1;
    }

    public String getRelationNameL2() {
        return this.relationNameL2;
    }

    public void setRelationNameL2(String relationNameL2) {
        this.relationNameL2 = relationNameL2;
    }

    public int getAge() {
        return this.age;
    }

    public void setAge(int age) {
        this.age = age;
    }

    public String getGender() {
        return this.gender;
    }

    public void setGender(String gender) {
        this.gender = gender;
    }

    public int getPartNumber() {
        return this.partNumber;
    }

    public void setPartNumber(int partNumber) {
        this.partNumber = partNumber;
    }

    public int getPartId() {
        return this.partId;
    }

    public void setPartId(int partId) {
        this.partId = partId;
    }

    public String getPartName() {
        return this.partName;
    }

    public void setPartName(String partName) {
        this.partName = partName;
    }

    public String getPartNameL1() {
        return this.partNameL1;
    }

    public void setPartNameL1(String partNameL1) {
        this.partNameL1 = partNameL1;
    }

    public int getPartSerialNumber() {
        return this.partSerialNumber;
    }

    public void setPartSerialNumber(int partSerialNumber) {
        this.partSerialNumber = partSerialNumber;
    }

    public String getAsmblyName() {
        return this.asmblyName;
    }

    public void setAsmblyName(String asmblyName) {
        this.asmblyName = asmblyName;
    }

    public String getAsmblyNameL1() {
        return this.asmblyNameL1;
    }

    public void setAsmblyNameL1(String asmblyNameL1) {
        this.asmblyNameL1 = asmblyNameL1;
    }

    public int getAcId() {
        return this.acId;
    }

    public void setAcId(int acId) {
        this.acId = acId;
    }

    public int getAcNumber() {
        return this.acNumber;
    }

    public void setAcNumber(int acNumber) {
        this.acNumber = acNumber;
    }

    public String getPrlmntName() {
        return this.prlmntName;
    }

    public void setPrlmntName(String prlmntName) {
        this.prlmntName = prlmntName;
    }

    public Object getPrlmntNameL1() {
        return this.prlmntNameL1;
    }

    public void setPrlmntNameL1(Object prlmntNameL1) {
        this.prlmntNameL1 = prlmntNameL1;
    }

    public String getPrlmntNo() {
        return this.prlmntNo;
    }

    public void setPrlmntNo(String prlmntNo) {
        this.prlmntNo = prlmntNo;
    }

    public String getDob() {
        return this.dob;
    }

    public void setDob(String dob) {
        this.dob = dob;
    }

    public String getDistrictValue() {
        return this.districtValue;
    }

    public void setDistrictValue(String districtValue) {
        this.districtValue = districtValue;
    }

    public String getDistrictValueL1() {
        return this.districtValueL1;
    }

    public void setDistrictValueL1(String districtValueL1) {
        this.districtValueL1 = districtValueL1;
    }

    public String getDistrictCd() {
        return this.districtCd;
    }

    public void setDistrictCd(String districtCd) {
        this.districtCd = districtCd;
    }

    public int getDistrictId() {
        return this.districtId;
    }

    public void setDistrictId(int districtId) {
        this.districtId = districtId;
    }

    public int getDistrictNo() {
        return this.districtNo;
    }

    public void setDistrictNo(int districtNo) {
        this.districtNo = districtNo;
    }

    public String getStateName() {
        return this.stateName;
    }

    public void setStateName(String stateName) {
        this.stateName = stateName;
    }

    public String getStateNameL1() {
        return this.stateNameL1;
    }

    public void setStateNameL1(String stateNameL1) {
        this.stateNameL1 = stateNameL1;
    }

    public int getStateId() {
        return this.stateId;
    }

    public void setStateId(int stateId) {
        this.stateId = stateId;
    }

    public String getStateCd() {
        return this.stateCd;
    }

    public void setStateCd(String stateCd) {
        this.stateCd = stateCd;
    }

    public boolean isActive() {
        return this.isActive;
    }

    public void setActive(boolean active) {
        this.isActive = active;
    }

    public String getFormType() {
        return this.formType;
    }

    public void setFormType(String formType) {
        this.formType = formType;
    }

    public String getRelationType() {
        return this.relationType;
    }

    public void setRelationType(String relationType) {
        this.relationType = relationType;
    }

    public String getCreatedDttm() {
        return this.createdDttm;
    }

    public void setCreatedDttm(String createdDttm) {
        this.createdDttm = createdDttm;
    }

    public String getModifiedDttm() {
        return this.modifiedDttm;
    }

    public void setModifiedDttm(String modifiedDttm) {
        this.modifiedDttm = modifiedDttm;
    }

    public String getGenderL1() {
        return this.genderL1;
    }

    public void setGenderL1(String genderL1) {
        this.genderL1 = genderL1;
    }

    public boolean isDeleted() {
        return this.isDeleted;
    }

    public void setDeleted(boolean deleted) {
        this.isDeleted = deleted;
    }

    public String getMobileNumber() {
        return this.mobileNumber;
    }

    public void setMobileNumber(String mobileNumber) {
        this.mobileNumber = mobileNumber;
    }

    public String getProcessType() {
        return this.processType;
    }

    public void setProcessType(String processType) {
        this.processType = processType;
    }

    public String getRelationLName() {
        return this.relationLName;
    }

    public void setRelationLName(String relationLName) {
        this.relationLName = relationLName;
    }

    public String getRelationLNameL1() {
        return this.relationLNameL1;
    }

    public void setRelationLNameL1(String relationLNameL1) {
        this.relationLNameL1 = relationLNameL1;
    }

    public String getRelationTypeL1() {
        return this.relationTypeL1;
    }

    public void setRelationTypeL1(String relationTypeL1) {
        this.relationTypeL1 = relationTypeL1;
    }

    public int getRevisionId() {
        return this.revisionId;
    }

    public void setRevisionId(int revisionId) {
        this.revisionId = revisionId;
    }

    public String getStatusType() {
        return this.statusType;
    }

    public void setStatusType(String statusType) {
        this.statusType = statusType;
    }

    public int getSectionNo() {
        return this.sectionNo;
    }

    public void setSectionNo(int sectionNo) {
        this.sectionNo = sectionNo;
    }

    public String getPollingDate() {
        return this.pollingDate;
    }

    public void setPollingDate(String pollingDate) {
        this.pollingDate = pollingDate;
    }

    public String getPsbuildingName() {
        return this.psbuildingName;
    }

    public void setPsbuildingName(String psbuildingName) {
        this.psbuildingName = psbuildingName;
    }

    public String getPartLatLong() {
        return this.partLatLong;
    }

    public void setPartLatLong(String partLatLong) {
        this.partLatLong = partLatLong;
    }

    public String getDisabilityAny() {
        return this.disabilityAny;
    }

    public void setDisabilityAny(String disabilityAny) {
        this.disabilityAny = disabilityAny;
    }

    public String getDisabilityType() {
        return this.disabilityType;
    }

    public void setDisabilityType(String disabilityType) {
        this.disabilityType = disabilityType;
    }

    public String getIsForm8Migration() {
        return this.isForm8Migration;
    }

    public void setIsForm8Migration(String isForm8Migration) {
        this.isForm8Migration = isForm8Migration;
    }

    public String getIsLocomotorDisabled() {
        return this.isLocomotorDisabled;
    }

    public void setIsLocomotorDisabled(String isLocomotorDisabled) {
        this.isLocomotorDisabled = isLocomotorDisabled;
    }

    public String getIsSpeechHearingDisabled() {
        return this.isSpeechHearingDisabled;
    }

    public void setIsSpeechHearingDisabled(String isSpeechHearingDisabled) {
        this.isSpeechHearingDisabled = isSpeechHearingDisabled;
    }

    public String getIsVisuallyImpaired() {
        return this.isVisuallyImpaired;
    }

    public void setIsVisuallyImpaired(String isVisuallyImpaired) {
        this.isVisuallyImpaired = isVisuallyImpaired;
    }

    public String getOtherDisability() {
        return this.otherDisability;
    }

    public void setOtherDisability(String otherDisability) {
        this.otherDisability = otherDisability;
    }

    public String getIsValidated() {
        return this.isValidated;
    }

    public void setIsValidated(String isValidated) {
        this.isValidated = isValidated;
    }

    public String getIsWheelchairRequired() {
        return this.isWheelchairRequired;
    }

    public void setIsWheelchairRequired(String isWheelchairRequired) {
        this.isWheelchairRequired = isWheelchairRequired;
    }

    public String getPwdMarkingFormType() {
        return this.pwdMarkingFormType;
    }

    public void setPwdMarkingFormType(String pwdMarkingFormType) {
        this.pwdMarkingFormType = pwdMarkingFormType;
    }

    public String getPwdMarkingRefNo() {
        return this.pwdMarkingRefNo;
    }

    public void setPwdMarkingRefNo(String pwdMarkingRefNo) {
        this.pwdMarkingRefNo = pwdMarkingRefNo;
    }

    public String getPwd() {
        return this.pwd;
    }

    public void setPwd(String pwd) {
        this.pwd = pwd;
    }

    public String getAadharStatus() {
        return this.aadharStatus;
    }

    public void setAadharStatus(String aadharStatus) {
        this.aadharStatus = aadharStatus;
    }

    public String getIsVip() {
        return this.isVip;
    }

    public void setIsVip(String isVip) {
        this.isVip = isVip;
    }

    public String getEpicDatetime() {
        return this.epicDatetime;
    }

    public void setEpicDatetime(String epicDatetime) {
        this.epicDatetime = epicDatetime;
    }

    public String getFullName() {
        return this.fullName;
    }

    public void setFullName(String fullName) {
        this.fullName = fullName;
    }

    public String getFullNameL1() {
        return this.fullNameL1;
    }

    public void setFullNameL1(String fullNameL1) {
        this.fullNameL1 = fullNameL1;
    }

    public String getRelativeFullName() {
        return this.relativeFullName;
    }

    public void setRelativeFullName(String relativeFullName) {
        this.relativeFullName = relativeFullName;
    }

    public String getRelativeFullNameL1() {
        return this.relativeFullNameL1;
    }

    public void setRelativeFullNameL1(String relativeFullNameL1) {
        this.relativeFullNameL1 = relativeFullNameL1;
    }

    public String getEmailId() {
        return this.emailId;
    }

    public void setEmailId(String emailId) {
        this.emailId = emailId;
    }

    public String getPinCode() {
        return this.pinCode;
    }

    public void setPinCode(String pinCode) {
        this.pinCode = pinCode;
    }

    public String getDvoterStatusType() {
        return this.dvoterStatusType;
    }

    public void setDvoterStatusType(String dvoterStatusType) {
        this.dvoterStatusType = dvoterStatusType;
    }

    public int getRevisionNo() {
        return this.revisionNo;
    }

    public void setRevisionNo(int revisionNo) {
        this.revisionNo = revisionNo;
    }

    public String getPhoto() {
        return this.photo;
    }

    public void setPhoto(String photo) {
        this.photo = photo;
    }

    public String getOldEpicNumber() {
        return this.oldEpicNumber;
    }

    public void setOldEpicNumber(String oldEpicNumber) {
        this.oldEpicNumber = oldEpicNumber;
    }

    public String getHouseNumber() {
        return this.houseNumber;
    }

    public void setHouseNumber(String houseNumber) {
        this.houseNumber = houseNumber;
    }

    public String getHouseNumberL1() {
        return this.houseNumberL1;
    }

    public void setHouseNumberL1(String houseNumberL1) {
        this.houseNumberL1 = houseNumberL1;
    }

    public Object getLocalityStreet() {
        return this.localityStreet;
    }

    public void setLocalityStreet(Object localityStreet) {
        this.localityStreet = localityStreet;
    }

    public Object getLocalityStreetL1() {
        return this.localityStreetL1;
    }

    public void setLocalityStreetL1(Object localityStreetL1) {
        this.localityStreetL1 = localityStreetL1;
    }

    public Object getTownVillage() {
        return this.townVillage;
    }

    public void setTownVillage(Object townVillage) {
        this.townVillage = townVillage;
    }

    public Object getTownVillageL1() {
        return this.townVillageL1;
    }

    public void setTownVillageL1(Object townVillageL1) {
        this.townVillageL1 = townVillageL1;
    }

    public String getLastUpdatedOn() {
        return this.lastUpdatedOn;
    }

    public void setLastUpdatedOn(String lastUpdatedOn) {
        this.lastUpdatedOn = lastUpdatedOn;
    }

    public String getIsOnlyMobileUpdation() {
        return this.isOnlyMobileUpdation;
    }

    public void setIsOnlyMobileUpdation(String isOnlyMobileUpdation) {
        this.isOnlyMobileUpdation = isOnlyMobileUpdation;
    }

    public String getTehsil() {
        return this.tehsil;
    }

    public void setTehsil(String tehsil) {
        this.tehsil = tehsil;
    }

    public String getTehsilL1() {
        return this.tehsilL1;
    }

    public void setTehsilL1(String tehsilL1) {
        this.tehsilL1 = tehsilL1;
    }

    public String getElectorType() {
        return this.electorType;
    }

    public void setElectorType(String electorType) {
        this.electorType = electorType;
    }

    public String getCountry() {
        return this.country;
    }

    public void setCountry(String country) {
        this.country = country;
    }
}

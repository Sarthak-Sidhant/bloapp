...
import in.gov.eci.bloapp.views.activity.newsir.activity.FormVerification.SelectPhotoActivity;
import in.gov.eci.bloapp.views.activity.newsir.model.VerifyPayload;
import java.util.ArrayList;
import java.util.List;

/* JADX INFO: loaded from: /tmp/dex_9.39/classes4.dex */
public class NonSirSelectPhotoListAdapter extends RecyclerView.Adapter<holder> {
    List<VerifyPayload> al;
    private Context context;
    List<VerifyPayload> filteredAl;

    public NonSirSelectPhotoListAdapter(List<VerifyPayload> al, Context context) {
        this.al = al;
    ...
    public void onBindViewHolder(holder holder2, final int i) {
        ...
        holder2.viewDetails.setOnClickListener(new View.OnClickListener() { // from class: in.gov.eci.bloapp.adapter.NonSirSelectPhotoListAdapter.1
            @Override // android.view.View.OnClickListener
            public void onClick(View view) {
                Intent intent = new Intent(NonSirSelectPhotoListAdapter.this.context, (Class<?>) SelectPhotoActivity.class);
                intent.putExtra("data", NonSirSelectPhotoListAdapter.this.al.get(i));
                intent.putExtra("from", "photo");
                NonSirSelectPhotoListAdapter.this.context.startActivity(intent);
            }
        });
